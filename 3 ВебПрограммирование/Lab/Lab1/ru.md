...
**Структура HTTP-ответа (HTTP Response)**
...
* Версия, код состояния, описание (например, `200 OK`, `404 Not Found`).

2.  **Заголовки** :

* `Content-Type`, `Set-Cookie`, `Server`, `Cache-Control`, `Date` и т.д.

3.  **Тело** :

* Содержит сам контент (HTML, JSON, изображения и т.д.).
...
**LESS, Sass, SCSS** — это  **CSS-препроцессоры** , расширяющие стандартный CSS: поддержка переменных, вложенности, функций и миксинов.
