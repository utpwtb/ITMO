 ...
 */
public class RemoveKeyClient extends CommandClient {
    private final NetWorkManager netWorkManager;

    /**
     * Конструктор.
     */
    public RemoveKeyClient(NetWorkManager netWorkManager) {
    ...
    public Request execute(String[] args, CommandMode mode) throws IOException {
        ...
        final byte[] serialized = netWorkManager.serializer(
                new Request("remove_key", new String[]{args[0], null}));
        if (serialized == null) {
        ...
        // 如果响应消息表明key不存在，则返回null终止操作
        if (responseMessage.equals("F")) {
            System.out.println("Указанный ключ не существует");
            return null;
        }

        // 第二阶段：执行删除操作
        return new Request("remove_key", args);
    }
